...
const VO2MAX_HISTORY = [44.8, 45.3, 45.9, 46.4, 46.9, 47.5, 47.9]; // ml/kg/min, oldest → newest
const VO2MAX_LABELS = ["6wk", "5wk", "4wk", "3wk", "2wk", "1wk", "Now"];
...
function Summary({ ai, aiLoading, onRegenerate, readiness, cycleDay, cycleLength, openCycle, openRecovery, onSignOut }) {
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div>
            ...
            <div style={{ fontSize: 26, fontWeight: 800, color: T.ink, marginTop: 4 }}>
              {currentVO2.toFixed(1)} <span style={{ fontSize: 13, color: T.sub, fontWeight: 400 }}>ml/kg/min</span>
            </div>
          </div>
          ...
          {VO2MAX_HISTORY.map((v, i) => {
            ...
            return (
              <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4, height: "100%", justifyContent: "flex-end" }}>
                <div style={{ width: "100%", height: `${h}%`, borderRadius: 4, background: i === VO2MAX_HISTORY.length - 1 ? T.accent2 : T.hair }} />
                <div style={{ fontSize: 8, color: T.sub }}>{VO2MAX_LABELS[i]}</div>
              </div>
            );
          })}
        </div>
...
- postWorkoutInsight: 1–2 sentences referencing a specific minute. weeklyNarrative: 1–2 sentences on the trend. liveCues: 5 short motivational mid-sprint voice cues. coreInsight: 1–2 sentences on today's core session and a concrete target for next time. vo2maxNote: 1–2 sentences on the VO2max trend and what it signals. weeklyPlan focus: short (≤5 words).`,
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#000", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      ...
      >
        <style>{`@keyframes breathePulse { 0%, 100% { transform: scale(0.65); opacity: 0.5; } 50% { transform: scale(1); opacity: 1; } }`}</style>
        {body}
      </div>
    </div>
  );
